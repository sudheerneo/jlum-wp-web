	...
	function hestia_category() {
		...
		if ( $category ) {
			echo '<a href="' . esc_url( get_category_link( $category[0]->term_id ) ) . '" title="' .
				 ...
				 sprintf( esc_html__( 'View all posts in %s', 'hestia' ),
...
if ( ! function_exists( 'hestia_comments_template' ) ) :
	...
	function hestia_comments_template() {
		...
		$args = array(
			...
			'comment_field' => '<div class="form-group label-floating is-empty"> <label class="control-label">' . esc_html__( 'What\'s in your mind?', 'hestia' ) . '</label><textarea id="comment" name="comment" class="form-control" rows="6" aria-required="true"></textarea><span class="hestia-input"></span> </div> </div>',
		);
		return $args;
	}
endif;
