...
class SIB_Widget_Subscribe extends WP_Widget
{
    ...
    */
    function form( $instance )
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'sib_form_list' ); ?>">
                <?php echo __('Form to use', 'sib_lang') . ':'; ?>
            </label>
            <select class="widefat" id="<?php echo $this->get_field_id('sib_form_list'); ?>" name="<?php echo $this->get_field_name('sib_form_list'); ?>">
            <?php
            foreach($sib_forms as $form)
            {
                ?>
                <option value="<?php echo $form['id']; ?>" <?php selected($sib_form_list, $form['id']); ?>><?php echo $form['title']; ?></option>
                <?php
            }
            ?>
            </select>
    ...
    function widget( $args, $instance )
    {
        ...
        SIB_Manager::$instance->generate_form_box($frmID);
        echo $after_widget;
    }
